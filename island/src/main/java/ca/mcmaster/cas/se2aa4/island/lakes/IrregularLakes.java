...
public class IrregularLakes {
    public Structs.Mesh lake(Structs.Mesh aMesh, int numLakes) {
        ...
        Irregular ireg = new Irregular();

        Structs.Mesh landAndLakeMesh = ireg.generateIsland(aMesh);
        ...
        //IslandBuilder.greenPolygons.addAll(landAndLakeMesh.getPolygonsList());
        ...
        for (Structs.Polygon p : landAndLakeMesh.getPolygonsList()) {
            if(!(greenPolygons.contains(p))&&!(bluePolygons.contains(p))&&(!lakePolygons.contains(p))){
                String RGB = 0 + "," + 0 + "," + 139;
                Structs.Property color = Structs.Property.newBuilder().setKey("rgb_color").setValue(RGB).build();
                tempList.add(Structs.Polygon.newBuilder(p).clearProperties().addProperties(color).build());
                bluePolygons.add(p);
